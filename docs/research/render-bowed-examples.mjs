...
registerHooks({
  resolve(specifier, context, nextResolve) {
    if (
      context.parentURL?.includes('/packages/audio/src/') &&
...
const { BowedRenderer } =
  await import('../../packages/audio/src/bowed-renderer.ts');
const rate = 48000;
...
function render(chords, name, length = 2) {
  ...
  chords.forEach((frequencies, index) => {
    frequencies.forEach((frequency) => {
      renderer.event({
        ...
        frequency,
        gain: 0.5 / Math.sqrt(frequencies.length),
        start: index * length,
  ...
  const samples = new Float32Array(
    Math.ceil((chords.length * length + 0.25) * rate),
  );
  ...
  for (let frame = 0; frame < samples.length; frame += block.length) {
    const output = block.subarray(
      0,
      Math.min(block.length, samples.length - frame),
    );
...
const report = {
  ...
  caveat:
    'Offline Node timings on this host are not browser deadline or physical-phone measurements. No subjective listening score is inferred.',
  solo: render([[65.4064], [130.8128], [261.6256], [437.25]], 'bowed-solo'),
  chords: render(
    [
      [261.6256, 329.6276, 391.9954],
      [220, 261.6256, 329.6276],
      [174.6141, 261.6256, 349.2282, 440],
      [195.9977, 246.9417, 293.6648, 349.2282, 440],
    ],
    'bowed-progression',
  ),
  microtonal: render(
    [
      [220, (220 * 7) / 6, (220 * 11) / 8, 330],
      [261.6256, 261.6256 * 2 ** (5 / 19), 261.6256 * 2 ** (11 / 19)],
    ],
    'bowed-arbitrary-frequencies',
  ),
  dense: render(
    [Array.from({ length: 24 }, (_, i) => 130.8128 * 2 ** (i / 19))],
    'bowed-dense',
    4,
  ),
};
