...
class StringDelay {
  ...
  private position = 0;
  constructor(length: number) { this.samples = new Float64Array(Math.ceil(length) + 8); }
  read(delay: number): number {
    ...
    return this.samples[left]! * (1 - fraction) + this.samples[right]! * fraction;
  }
...
class BowedPlayer {
  ...
  constructor(frequency: number, rate: number, seed: number, expressive: boolean) {
    this.rate = rate;
    this.seed = seed >>> 0;
    ...
    this.filterDelay = Math.atan2(this.pole * Math.sin(omega), 1 - this.pole * Math.cos(omega)) / omega;
    this.position = Math.max(0.12 + 0.025 * this.random(), 1.1 / (this.period - this.filterDelay));
  ...
  tick(contact: number): number {
    ...
    if (this.control === 0) {
      ...
      const vibrato = this.expressive
        ? 0.002 * Math.min(1, time / 0.35) * Math.sin(2 * Math.PI * this.vibratoRate * time + this.vibratoPhase)
        : 0;
      ...
      const gesture = this.expressive ? 0.025 * Math.sin(2 * Math.PI * 0.8 * time + this.motionPhase) + 0.012 * this.motion : 0;
    ...
    this.filtered = this.pole * this.filtered + (1 - this.pole) * this.bridge.read(this.bridgeLength);
...
class LowPass {
  ...
  constructor(frequency: number, rate: number, q: number) {
    const omega = 2 * Math.PI * frequency / rate;
    const cosine = Math.cos(omega), alpha = Math.sin(omega) / (2 * q);
...
export class BowedString {
  ...
  constructor(frequency: number, sampleRate: number, seed = 1, expressive = true) {
    ...
    const bodyScale = 0.65 + 0.35 * Math.min(1, Math.max(0, Math.log2(frequency / 130.8128)));
  ...
  tick(contact = 1): number {
    ...
    for (let sample = 0; sample < 2; sample++) {
      output = (this.players[0]!.tick(contact) + this.players[1]!.tick(contact)) * Math.SQRT1_2;
