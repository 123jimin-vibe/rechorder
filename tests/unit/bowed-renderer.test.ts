...
import {
  maximumVoices,
  type BowedEvent,
} from '../../packages/audio/src/bowed-protocol';
...
describe('streaming bowed renderer', () => {
  it('renders the same scheduled sound across different render-quantum sizes', () => {
    const a = new BowedRenderer(rate, () => {}),
      b = new BowedRenderer(rate, () => {});
    a.event(note(1, 437.25, 0.10123));
    b.event(note(1, 437.25, 0.10123));
    const output = render(a, 0, rate, 128);
    expect(output).toEqual(render(b, 0, rate, 192));
    expect(
      output.subarray(0, Math.round(0.10123 * rate)).every((x) => x === 0),
    ).toBe(true);
  ...
  it('cancels a future voice without a late onset and sends one completion', () => {
    const ended: number[] = [],
      renderer = new BowedRenderer(rate, (id) => ended.push(id));
  ...
  it('releases one overlapping voice without shortening another', () => {
    const ended: number[] = [],
      renderer = new BowedRenderer(rate, (id) => ended.push(id));
    renderer.event(note(1, 440, 0, 4));
    renderer.event(note(2, 523.251, 0, 4));
  ...
  it('keeps a dense arbitrary-frequency chord finite, audible and below the ceiling', () => {
    ...
    for (let i = 0; i < 24; i++)
      renderer.event(note(i + 1, 130.8128 * 2 ** (i / 19)));
    const output = render(renderer, 0, rate * 2);
    expect(
      output.every((x) => Number.isFinite(x) && Math.abs(x) <= 0.881),
    ).toBe(true);
    const sustain = output.subarray(rate / 4, (rate * 3) / 4);
    expect(
      Math.sqrt(sustain.reduce((sum, x) => sum + x * x, 0) / sustain.length),
