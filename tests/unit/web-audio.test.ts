...
describe('native audio worklet lifecycle', () => {
  it('resumes in the gesture and becomes schedulable only after the module loads', async () => {
    let loaded!: () => void;
    const { context, nodes } = nativeHarness(() => new Promise((resolve) => { loaded = resolve; }));
    const driver = createWebAudioDriver(), ready = driver.resume();
    ...
    expect(nodes[0]!.port.postMessage).toHaveBeenCalledWith(expect.objectContaining({
      type: 'start', frequency: 437.25, start: 0.2, end: 1.2,
    }));
  ...
  it('retries a failed module fetch without creating a second context', async () => {
    ...
    context.audioWorklet.addModule.mockRejectedValueOnce(new Error('module unavailable'));
  ...
  it('does not create a renderer if disposed during initialization', async () => {
    let loaded!: () => void;
    const { nodes } = nativeHarness(() => new Promise((resolve) => { loaded = resolve; }));
    const driver = createWebAudioDriver(), ready = driver.resume();
    await driver.close();
    loaded();
  ...
  it('finishes voices after a processor failure and rebuilds on the next gesture', async () => {
    ...
    const { context, nodes } = nativeHarness();
    const driver = createWebAudioDriver(); await driver.resume();
    const ended = vi.fn(); driver.schedule(440, 0.5, 0, 1, ended);
