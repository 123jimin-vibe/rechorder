...
import {
  BowedString,
  bowReflection,
} from '../../packages/audio/src/string-model';
...
function cents(samples: Float32Array, frequency: number, rate: number): number {
  ...
  const begin = Math.floor(rate * 0.4),
    window = Math.floor(rate * 0.25);
  ...
  for (
    let lag = Math.max(2, Math.floor(expected) - 8);
    lag <= Math.ceil(expected) + 8;
    lag++
  )
...
describe('physical bowed strings', () => {
  it.each([44100, 48000, 96000])(
    'keeps continuous pitches in tune at %s Hz',
    (rate) => {
      for (const frequency of [
        32.7032, 65.4064, 130.8128, 277.1826, 437.25, 987.7666, 1975.5332,
      ]) {
        const samples = render(frequency, rate, 1);
        expect(samples.every(Number.isFinite)).toBe(true);
        expect(
          Math.abs(cents(samples, frequency, rate)),
          `${frequency} Hz at ${rate}`,
        ).toBeLessThan(8);
  ...
  it('models a sticking/slipping bow and rings down when contact is removed', () => {
    ...
    const rate = 48000,
      model = new BowedString(220, rate, 1, false);
    const bowed = Float32Array.from({ length: rate * 2 }, () => model.tick());
    const released = Float32Array.from({ length: rate * 2 }, () =>
      model.tick(0),
    );
    expect(rms(bowed, rate, rate * 2)).toBeGreaterThan(
      rms(bowed, rate / 2, rate) * 0.6,
    );
    expect(rms(released, 0, 1000)).toBeGreaterThan(0.001);
    expect(rms(released, rate, rate * 2)).toBeLessThan(
      rms(bowed, rate, rate * 2) * 0.03,
    );
    const unbowed = new BowedString(220, rate);
    expect(
      Float32Array.from({ length: rate }, () => unbowed.tick(0)).every(
        (x) => x === 0,
      ),
    ).toBe(true);
  });
  ...
  it.each([32.7032, 65.4064, 130.8128, 440, 1975.5332])(
    'sustains speaker-band energy at %s Hz',
    (frequency) => {
  ...
  it('gives repeated notes independent bow motion, with reproducible diagnostic seeds', () => {
    ...
    expect(
      rms(Float32Array.from(first, (x, i) => x - next[i]!)),
  ...
  it('keeps the body resonances stable at different output rates', () => {
    for (const rate of [8000, 44100, 48000, 96000, 192000]) {
      ...
      const impulse = Float32Array.from({ length: rate }, (_, i) =>
        body.tick(i === 0 ? 1 : 0),
      );
...
describe('transparent output peak protection', () => {
  it('preserves a quiet chord mixture exactly apart from its lookahead delay', () => {
    ...
    const input = Float64Array.from(
      ...
      (_, i) =>
        0.25 *
        (Math.sin((2 * Math.PI * 437 * i) / 48000) +
          Math.sin((2 * Math.PI * 563 * i) / 48000)),
    );
    for (let i = 0; i < input.length; i++)
      expect(limiter.tick(input[i]!)).toBe(
        i < limiter.latency ? 0 : input[i - limiter.latency],
      );
  });
  it('contains abrupt overlapping peaks and clears delayed sound on stop', () => {
    ...
    limiter.clear();
    for (let i = 0; i < limiter.latency * 2; i++)
      expect(limiter.tick(0)).toBe(0);
  });
});
