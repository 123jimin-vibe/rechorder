...
## Sonority descriptors and acoustic ranking — t0036, t0037

- `measureSonority` accepts any finite positive frequency collection and returns pair-averaged roughness from six harmonic partials with 1/h amplitudes, register-free spectral pitch-class harmonicity (Milne: twelve partials, h^-0.67 amplitude, 6.8-cent smearing, best cosine fit to one harmonic series), and frequency-span cents. `voiceMotion` measures minimum ordered movement in cents with an explicit 300-cent entry/exit cost. None requires a chord definition or twelve-tone pitch class. These model descriptions must not be presented as calibrated preference scores.
- `recommendChords` returns one ranked list. Its tonal terms are unchanged; an acoustic term measured on each chosen voicing (harmonicity relative to a major triad, minus roughness in excess of it, with the bass shifted to C4) lets consonant unconventional chords climb and pushes clusters, symmetric and over-stacked chords down without re-ranking idiomatic harmony. The pool adds bounded degree edits from independent major/minor seeds and adjacent user chords to the catalogue, requires at least three distinct pitch classes, excludes the harmony of adjacent or replaced chords, and voices every harmony before scoring. Results remain auditionable and persistable conventional chords; arbitrary-note storage remains future work.
