...
- `recommendChords` defaults to an unranked Varied selection representing blend, roughness contrast, voice continuity and tonal interpretation. Blend and Contrast lenses use concrete sounding voicings and score the roughness trajectory and voice movement relative to the adjacent chords, with an explicit relative score; Tonal preserves the preceding conventional behavior. Exploratory lenses add bounded degree edits from independent major/minor seeds and adjacent user chords to the catalogue pool. Every lens requires at least three distinct pitch classes and excludes the harmony of adjacent or replaced chords, because pair-averaged roughness is not comparable across cardinalities and zero movement is not a suggestion (t0037). Results remain auditionable and persistable conventional chords; arbitrary-note storage remains future work.
