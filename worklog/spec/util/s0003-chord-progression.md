...
- Default to Varied suggestions that represent different measurable aims without asserting one universal chord score. Let users select Blend, Contrast, or Tonal ranking. The Tonal lens ranks by function first: key role and root motion decide, voice-leading smoothness breaks ties. It offers inversions only to continue a started bass line; ordinary root motion keeps roots in the bass. Returning to the tonic is never penalized. Weighted-lens lists are in score order and each bar agrees with its position (t0029).
...
- Place one compact recommendation panel beside the timeline/builder, with Next, Replace, Between and Bass modes and four choices. Next appends at the end; Replace targets the selected stored chord; Between inserts after the selection only when a following chord exists; Bass offers other chord types/roots over the candidate's sounding bass.
- Disable unavailable targets. Show the target chords/bass and any sufficiently supported inferred key as a possibility. A suggestion's labelled preview auditions without editing the candidate or timeline. Its adjacent Add, Replace, Insert or Use action applies directly without an additional audition. Tonal choices retain Roman-numeral and move labels; acoustic choices show modeled roughness, span and voice movement. Keep the theoretical reasons in the tooltip. Show an accessible relative-score bar only for the three weighted lenses; Varied choices are deliberately unranked and have no bar.

### Open-ended assessment — t0036

- Measure sounding notes independently of chord names: average pairwise spectral roughness under an explicit six-harmonic, 1/h amplitude model; register span in cents; and minimum ordered voice movement in cents with a 300-cent voice entry/exit cost. These are descriptors, not listener ratings or universal musical quality.
- Blend favors a smaller roughness change from the adjacent chords and less actual voice movement; Contrast favors a larger roughness change and more movement. Both are relative to the neighbors, falling back to the chord's own roughness only when nothing surrounds the target (t0037). Both score the concrete voicing after considering inversions and register; expose their component terms. Tonal ranking remains selectable with its previous score and interpretation. Without a chosen lens, Varied offers alternatives from the Blend total, the Contrast total, voice continuity and tonal interpretation, preferring distinct roots; it assigns no comparative score to their order.
- The exploratory candidate pool includes the conventional root/quality catalogue plus bounded add/omit note edits from independent major/minor seeds and adjacent user chords. Every lens applies one declared candidate policy: at least three distinct pitch classes, and never the harmony of an adjacent or replaced chord (t0037). All generated results use the editor's persistable recipe operations. Arbitrary intervals outside that grammar still require a future first-class note-collection model.
