...
- Blend favors lower modeled roughness and less actual voice movement to adjacent chords. Contrast favors a larger roughness change and more movement. Both score the concrete voicing after considering inversions and register; expose their component terms. Tonal ranking remains selectable with its previous score and interpretation. Without a chosen lens, Varied offers alternatives from blend, roughness contrast, voice continuity and tonal interpretation, preferring distinct roots; it assigns no comparative score to their order.
- The exploratory candidate pool includes the conventional root/quality catalogue plus bounded add/omit note edits from independent major/minor seeds and adjacent user chords. All generated results use the editor's persistable recipe operations. Arbitrary intervals outside that grammar still require a future first-class note-collection model.
