...
- Rank by function first: key role and root motion decide, voice-leading smoothness breaks ties, and one acoustic term measured on the chosen voicing separates sonorities the tonal terms cannot (t0037). Offer inversions only to continue a bass line the user started with an inverted chord; ordinary root motion keeps roots in the bass. Returning to the tonic is never penalized. The list is in score order and each bar agrees with its position (t0029).
...
- Disable unavailable targets. Show the target chords/bass and any sufficiently supported inferred key as a possibility. A suggestion's labelled preview auditions without editing the candidate or timeline. Its adjacent Add, Replace, Insert or Use action applies directly without an additional audition. Label each suggestion with its Roman numeral when the key is known and its strongest move (bass line or root move); keep the complete reason list and the modeled harmonicity, roughness and voice movement in the tooltip. Show a compact accessible bar for each suggestion's normalized relative heuristic score; present it as comparative ranking, not probability.

### Open-ended assessment — t0036, t0037

- Measure sounding notes independently of chord names: average pairwise spectral roughness under an explicit six-harmonic, 1/h amplitude model; spectral pitch-class harmonicity against a single harmonic series; register span in cents; and minimum ordered voice movement in cents with a 300-cent voice entry/exit cost. These are descriptors, not listener ratings or universal musical quality.
- One ranked list, no selectable lenses (t0037 replaced t0036's Varied, Blend and Contrast lenses, which selected descriptor extremes and surfaced dyads, doublings, repeats and symmetric chords). The acoustic term of each chosen voicing adds harmonicity relative to a major triad and subtracts roughness in excess of it, measured with the bass shifted to C4 so register chosen for voice leading does not change it. Conventional harmony ranks as before; unconventional chords climb only when consonant, and clusters and over-stacked chords fall. Weights are documented in `docs/chord-recommendations.md`.
- The candidate pool includes the conventional root/quality catalogue plus bounded add/omit note edits from independent major/minor seeds and adjacent user chords, under one declared policy: at least three distinct pitch classes, and never the harmony of an adjacent or replaced chord. Every harmony is voiced and measured; nothing is preselected out before the acoustic term. All generated results use the editor's persistable recipe operations. Arbitrary intervals outside that grammar still require a future first-class note-collection model.
